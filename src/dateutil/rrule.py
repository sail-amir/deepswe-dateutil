...
class _iterinfo(object):
    ...
    def _rebuild_year(self, year):
        ...
        self.wdaymask = WDAYMASK[self.yearweekday:]
    ...
    def _add_previous_year_week(self, year, no1wkst):
        rr = self.rrule
        # Check last week number of last year as well. If no1wkst is 0,
        # either the year started on week start, or week number 1 got days
        # from last year, so this year has no days from the previous last week.
    ...
    def _rebuild_nweekday_mask(self, month):
        ...
        # Weekly frequency won't get here, so we may not care about
        # cross-year weekly periods.
    ...
    def _rebuild_easter_mask(self, year):
        ...
        for offset in self.rrule._byeaster:
...
class _rrulestr(object):
    ...
    def _handle_BYWEEKDAY(self, rrkwargs, name, value, **kwargs):
        ...
        """
        rrkwargs["byweekday"] = [self._parse_rfc_weekday(wday)
                                 for wday in value.split(',')]

    def _parse_rfc_weekday(self, wday):
        if '(' in wday:
